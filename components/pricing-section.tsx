...
export function PricingSection() {
  ...
  const pricingTiers = tierOrder.map((tier) => ({ ...pricing.tiers[tier.key], highlighted: tier.highlighted }))
              ...
              <CardHeader className="text-center pb-8">
                ...
                <div className="mt-4">
                  <span className="text-4xl font-bold">
                    {tier.price === pricing.tiers.custom.price ? (
                      <span className="text-3xl">{tier.price}</span>
                    ) : (
                      <>
                        <span className="text-lg font-normal text-muted-foreground">{pricing.fromLabel}</span>
                        {tier.price}
                      </>
                    )}
                  </span>
